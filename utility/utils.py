...
def loss(yhat, y, mean=True, add_other_losses=True):
    ...
    if mean:
        ...
        if add_other_losses:
            tf.compat.v1.add_to_collection('losses', loss_1)
...
def weights_spectral_norm_linear(weights, u=None, it=1, update_collection=None, 
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        if u is None:
            u = tf.compat.v1.get_variable('u', shape=[1, weights_shape[-1]], 
                                initializer=tf.truncated_normal_initializer(), trainable=False)
        ...
        tf.compat.v1.add_to_collection('weights_after_spectral_norm_lin', w_norm)
