...
by Arash Rahnama
Modified and Updated on Nov 24
by Bernardo Aquino
"""
...
def linear(x, output_size, scope_name='linear', spectral_norm=True, update_collection=None, 
           l2_norm=False, weight_decay=0, xavier=True, rho=1.0, reuse=True):
    ...
    with tf.compat.v1.variable_scope(scope_name, reuse=tf.compat.v1.AUTO_REUSE):
        ...
        if xavier:
            weights = tf.compat.v1.get_variable('weights', [input_size, output_size], tf.float32, 
                                      initializer=tf.contrib.layers.xavier_initializer())
        else:
            weights = tf.compat.v1.get_variable('weights', [input_size, output_size], tf.float32, 
                                      initializer=tf.random_normal_initializer(stddev=0.02))
            
        bias = tf.compat.v1.get_variable('bias', output_size, tf.float32, initializer=tf.constant_initializer(0))        
