...
def graph_wrapper(arch, num_classes=NUM_CLASSES, adv='basic', eps=0.3, weight_decay=0, update_collection=None, 
    ...
    input_data = tf.compat.v1.placeholder(tf.float32, shape=[None, IMAGE_H, IMAGE_W, num_channels], name='InputData')
    input_labels = tf.compat.v1.placeholder(tf.int64, shape=[None], name='InputLabels')
    ...
    with tf.control_dependencies(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)):
        if opt_method == 'adam' or num_channels == 1:
            opt_step = tf.compat.v1.train.AdamOptimizer(0.001).minimize(total_loss)
        else:
    ...
    if save_dir is not None:
        ...
        if not os.path.isdir(save_dir):
            ...
            tf.compat.v1.summary.scalar('accuracy', total_acc)
            
            ## histograms for variables that are being trained
            if save_histograms:
                for i in tf.compat.v1.trainable_variables():
                    tf.compat.v1.summary.histogram(i.op.name, i)
            
            ## merge the summaries and write them to save_dir
            merged = tf.compat.v1.summary.merge_all()
            train_writer = tf.compat.v1.summary.FileWriter(os.path.join(save_dir, 'train'))
            graph_writer = tf.compat.v1.summary.FileWriter(os.path.join(save_dir, 'graph'), 
                                                 graph=tf.compat.v1.get_default_graph())
            valid_writer = tf.compat.v1.summary.FileWriter(os.path.join(save_dir, 'validation'))
...
def graph_predict(x, arch, load_dir, labs=None, num_classes=NUM_CLASSES, num_channels=NUM_CHANNELS, delta=1.0, 
                  batch_normal=False, l2_norm=False, spectral_norm=True, weight_decay=-1, load_epoch=None, norm=2, 
